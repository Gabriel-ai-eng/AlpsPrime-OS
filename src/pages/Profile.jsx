      ...
      <div className="max-w-3xl mx-auto px-4 lg:px-6">
        ...
        <div className="flex items-end justify-between -mt-12 mb-6 relative z-10">
          <div className="relative">
            {/* Círculo da foto em preto */}
            <div className="w-24 h-24 rounded-full bg-black p-1 flex items-center justify-center shadow-xl relative overflow-hidden ring-4 ring-background">
              <div className="w-full h-full rounded-full bg-zinc-900 overflow-hidden relative">
                {user.profile_picture_url ? (
                  ...
                  <User className="w-10 h-10 text-background absolute inset-0 m-auto" />
                )}
              </div>
            </div>
            {!readOnly && (
              <>
                {/* Ícone da câmera em cinza com o fundo do botão preto */}
                <button
                  onClick={() => avatarInputRef.current?.click()}
                  className="absolute bottom-0 right-0 w-8 h-8 rounded-full bg-black flex items-center justify-center border-2 border-background hover:bg-zinc-900 transition-colors"
                >
